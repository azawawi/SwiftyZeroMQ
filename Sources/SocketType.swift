...
extension SwiftyZeroMQ {
    ...
    /**
        ...
        - Request-reply pattern: `.request`, `.reply`, `.router`, `.dealer`
          The request-reply pattern is used for sending requests from a `.request`
          client to one or more `.reply` services, and receiving subsequent
          replies to each request sent. The request-reply pattern is formally
          defined [here](http://rfc.zeromq.org/spec:28).
        ...
        - Pipeline pattern: `.push`, `.pull`
          ...
          the pipeline is connected to at least one node. When a pipeline stage
          is connected to multiple nodes data is round-robined among all
        ...
        - Exclusive pair pattern: `.pair`
          ...
          the inproc transport. The exclusive pair pattern is formally defined
          [here](http://rfc.zeromq.org/spec:31).
